...
APPS.push(() => {
  ...
  function main() {
      ...
      shader.createImageTexture({ name: 'source', flipFlop: true, output: true, src: WOMAN_JPG, callback: (texture) => {
  ...
  function timeLoop(func) {
      ...
      const loop = () => {
          ...
          const frameLength = frames.reduce((t, x) => t + x, 0) / frames.length;
          const fps = 1000 / Math.max(1, frameLength);
          fpsEl.textContent = `FPS: ${Math.floor(fps)}`;
          lastTime = time;
          setTimeout(loop, 0);
      };
      loop();
